...
const firebaseAuth = {
  ...
  async createUserWithEmailAndPassword(email, password, name, phone) {
    ...
    const otp = firebaseDB.generateOTP();
    firebaseDB.storeOTP(email, otp, "verify");
    const user = {
      uid, email, name, phone,
      plan: "free",
      createdAt: new Date().toISOString(),
      isAdmin: email === "maposacourage41@gmail.com",
      emailVerified: false,
      verifyToken,
      twoFAEnabled: false,
      lastLogin: null,
    ...
    await sendEmail(email, name,
      "Verify your ScottyHub account",
      `Welcome to ScottyHub! Your email verification code is:`,
      otp
    );
    firebaseDB.logActivity(uid, email, "REGISTER", `New account created`);
    return { ...user, needsVerification: true };
  },
  async signInWithEmailAndPassword(email, password) {
    const u = firebaseDB.users[email];
    if (!u || u.password !== password) throw new Error("Invalid email or password.");
    if (u.status === "banned") throw new Error("This account has been suspended. Contact support.");
    const user = { uid: u.uid, email: u.email, name: u.name, phone: u.phone, plan: u.plan, isAdmin: u.isAdmin, createdAt: u.createdAt, emailVerified: u.emailVerified, twoFAEnabled: u.twoFAEnabled };
    // Update login stats
    firebaseDB.users[email].lastLogin = new Date().toISOString();
    firebaseDB.users[email].loginCount = (u.loginCount || 0) + 1;
    firebaseDB.save();
    firebaseDB.logActivity(u.uid, email, "LOGIN", `Signed in successfully`);
    // If 2FA enabled, return flag
    if (u.twoFAEnabled) {
      const otp = firebaseDB.generateOTP();
      firebaseDB.storeOTP(email, otp, "2fa");
      await sendEmail(email, u.name, "ScottyHub 2FA Code", "Your login verification code:", otp);
      return { ...user, needs2FA: true };
    }
  ...
  async verifyEmailOTP(email, otp) {
    const valid = firebaseDB.verifyOTP(email, otp, "verify");
    if (!valid) throw new Error("Invalid or expired code. Check your email.");
    firebaseDB.users[email].emailVerified = true;
    firebaseDB.save();
    const u = firebaseDB.users[email];
    const user = { uid: u.uid, email: u.email, name: u.name, phone: u.phone, plan: u.plan, isAdmin: u.isAdmin, createdAt: u.createdAt, emailVerified: true, twoFAEnabled: u.twoFAEnabled };
    firebaseDB.logActivity(u.uid, email, "EMAIL_VERIFIED", "Email verified");
    this.currentUser = user;
    localStorage.setItem("sh_current", JSON.stringify(user));
    return user;
  },
  async verify2FA(email, otp) {
    const valid = firebaseDB.verifyOTP(email, otp, "2fa");
    if (!valid) throw new Error("Invalid or expired 2FA code.");
    const u = firebaseDB.users[email];
    const user = { uid: u.uid, email: u.email, name: u.name, phone: u.phone, plan: u.plan, isAdmin: u.isAdmin, createdAt: u.createdAt, emailVerified: u.emailVerified, twoFAEnabled: u.twoFAEnabled };
    firebaseDB.logActivity(u.uid, email, "2FA_SUCCESS", "2FA verified");
    this.currentUser = user;
    localStorage.setItem("sh_current", JSON.stringify(user));
    return user;
  },
  async resendOTP(email, type = "verify") {
    ...
    const otp = firebaseDB.generateOTP();
    firebaseDB.storeOTP(email, otp, type);
    await sendEmail(email, u.name, type === "2fa" ? "ScottyHub 2FA Code" : "Verify your ScottyHub email", "Your verification code:", otp);
    return true;
  },
  async signOut() {
    if (this.currentUser) firebaseDB.logActivity(this.currentUser.uid, this.currentUser.email, "LOGOUT", "Signed out");
...
const EMAILJS_PUBLIC_KEY = "jBQcleV5mpzLv4lxW"; // replace at emailjs.com
